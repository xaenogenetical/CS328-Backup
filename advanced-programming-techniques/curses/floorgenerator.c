...
void draw_dungeon()
{
    srand((unsigned)time(NULL)); // cast to unsigned gets the compiler to stop yelling at me
    ...
    int p_r = rand() % 21;
    int p_c = rand() % 80;
    while (level[p_r][p_c] != FLOOR)
    {
        p_r = rand() % 21;
        p_c = rand() % 80;
        continue; // make sure player starts on a floor tile
    }
    player_loc.x = p_c;
    player_loc.y = p_r;
...
void set_hardness()
{
    for (int row = 0; row < 21; row++)
    {
        ...
        else
        {
            for (int col = 0; col < 80; col++)
            {
                lev.hardness_map[row][col] = (rand() % 253) + 1;
            }
