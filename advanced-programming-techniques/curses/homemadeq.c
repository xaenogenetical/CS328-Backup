...
void insert(queue *pq, void *key)
{
    ...
    pq->data = malloc(sizeof(node_t) * (pq->size + 1));
...
void destroy_queue(queue *pq) // VERY IMPORTANT!!! USER MUST FREE DATAS TO PREVENT MEMORY LEAKS
{
    datas_to_free = (node_t **)realloc(datas_to_free, sizeof(node_t) * (data_idx + 1));
    datas_to_free[data_idx] = pq->data;
    data_idx++;
    free(pq);
}
