...
{
    return get_enemy_speed(b) - get_enemy_speed(a); // Its set up this way to convert the queue to a max-priority queue instead of mine without me having to redo the whole structure
}
...
{ // Even though abilities is a char, it should be passed as a 4-bit number
    ...
    // Puts the speed into the empty char
...
int enemy_move(character_t *enemy)
{
    ...
    int target_x = enemy->player_spotted_at.x;
    int target_y = enemy->player_spotted_at.y;
    int current_x = enemy->locale.x;
    int current_y = enemy->locale.y;
    lr = target_x - current_x;
    ud = target_y - current_y;
    lr = lr < 0 ? -1 : 1;
    ud = ud < 0 ? -1 : 1;
...
int do_turn(queue *q, character_t **enemies, character_t *player, int turn_count)
{
    ...
    while (!is_empty(q))
    {
        ...
        int outcome = enemy->is_player ? player_move() : enemy_move(enemy);
        if (outcome == 1)
        {
            destroy_queue(q);
            return 1;
